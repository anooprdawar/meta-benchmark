...
def test_merge_deep_diverge_p95_within_target(diverged_repo):
    ...
    # Merge can only run once (idempotent repeat would be a no-op).
    # Report single elapsed as p50/p95/p99 for consistent extraction.
    print(f"\nmerge deep diverge — p50={elapsed:.3f}s p95={elapsed:.3f}s p99={elapsed:.3f}s")
